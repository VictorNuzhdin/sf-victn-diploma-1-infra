...
LOG_FILE="kuber_setup_worker.log"
...
echo "--KUBERNETES_WORKER_NODE_BASE_SETUP__BEGINS.."
##--STEP00: system setup (all nodes)
##..setting timezone
sudo timedatectl set-timezone Asia/Omsk
#
echo "$(date +'%Y-%m-%dT%H:%M:%S') :: --KUBERNETES_WORKER_NODE_BASE_SETUP__BEGINS" >> $LOG_PATH
#
...
#
echo "--KUBERNETES_WORKER_NODE_BASE_SETUP__DONE!"
echo "$(date +'%Y-%m-%dT%H:%M:%S') :: --KUBERNETES_WORKER_NODE_BASE_SETUP__DONE" >> $LOG_PATH
echo "" >> $LOG_PATH
